...
테스트용이기 때문에 임의의 값을 넣어두고 코드를 작성했다.

아래 그림을 보면 각 `MemberService`와 `BookService`에게 어떤 값을 넘겨줄지 예상할 수 있다.
...
port 번호는 테스트용으로 2개의 서버를 돌리며 테스트해야하기때문에 기존 8080과 다르게 바꿔서 진행했다.
...
정리하자면
`@Value`를 이용해 외부(`properties`)에 있는 값을 주입받아 필드에 할당함으로써, TestService에서 이 값을 사용하는 것 이다.
