...
먼저 dependencies를 적용시켜주고, 아래 버킷 이름과, 엑세스 키와 시크릿 키를 발급 받아 넣어준다.
...
`@Value`를 이용해 `yml`에 작성한 각 값들을 넣어줬다.

`amazonS3Client()`를 이용하여 Amazon S3 클라이언트를 생성하고 구성했는데, 엑세스 키와 시크릿 키를 사용하여 AWS 자격 증명을 설정하고, 지정된 region에 대한 S3 클라이언트를 생성하여 반환한다.

코드를 자세히 살펴보면, `BasicAWSCredentials` 클래스를 사용하여 **AWS 자격 증명 객체를 생성**한다. 해당 객체는 AWS 서비스에 대한 접근을 인증하는 데 사용된다.

그 다음에는 `AmazonS3ClientBuilder` 클래스를 사용하여 **AmazonS3 클라이언트를 생성**하는데, 이 클라이언트는 앞에서 설정한 AWS 자격증명과 region을 사용하여 S3 서비스와 상호 작용한다.
...
`S3Uploader` 클래스는 AWS S3에 파일을 업로드하는 데 사용된다.
...
`private upload()` 메서드는 **실제로 파일 업로드 작업을 진행**하는 한다. 업로드 파일의 이름과 경로를 결정하고, S3에 파일을 업로드한 다음 해당 파일의 URL을 반환해준다.

`putS3()` 메서드는 **파일을 실제로 S3에 업로드**를 한다. **파일을 S3 버킷에 업로드**하고, **공개 읽기 권한을 부여**한다.

`removeNewFile()` 메서드는 **업로드 후에 로컬 파일을 삭제**하고, **파일 삭제 여부에 대한 로그를 기록**한다.

`convert()`메서드는 `MultipartFile`을 `File`**로 변환**한다. 

즉, 해당 클래스를 통해 **클라이언트에서 업로드한 파일을 AWS S3에 업로드**하고, **업로드된 파일에 대한 공개 URL을 반환**하는 것이다.
...
`@RequestPart`를 사용하여 **클라이언트에서 전송된** `image`를 *`MultipartFile`* **객체로 수신**한다. 즉, 업로드 된 이미지 파일을 나타낸다.

그럼 `customRecipeService.saveImageCustomRecipe()` 를 살펴보자.
...
먼저 `s3Uploader.upload(image,"images");` 를 이용하여 이미지를 S3에 업로드하고, 업로드 된 이미지의 URL을 `storedFileName` 변수에 저장했다.
